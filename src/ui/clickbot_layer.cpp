...
bool ClickbotLayer::init() {
	...
	CCMenuItemSpriteExtra* btn = CCMenuItemExt::createSpriteExtra(spr, [this](CCMenuItemSpriteExtra* sender) { ClickbotLayer::openClickSettings(sender); } );
	...
	btn = CCMenuItemExt::createSpriteExtra(spr, [this](CCMenuItemSpriteExtra* sender) { ClickbotLayer::openClickSettings(sender); } );
	...
	volumeSlider = SliderNode::create(
	...
	pitchLabel = CCLabelBMFont::create(fmt::format("Master Pitch ({:.1f})", pitch).c_str(), "goldFont.fnt");
...
bool ClickSettingsLayer::init(std::string button, geode::Popup* layer) {
	...
	pitchLabel = CCLabelBMFont::create(fmt::format("Pitch ({:.1f})", settings.pitch).c_str(), "goldFont.fnt");
	...
	btn = CCMenuItemExt::createSpriteExtra(
		spr,
		[this, button](CCMenuItemSpriteExtra* sender) {
