...
$execute {
    ...
    if (!g.mod->setSavedValue("button_defaults5", true)) {
        ...
        g.mod->setSavedValue("button_advance_frame_pos_y",
                             winSize.height - 50.f);
...
void ButtonEditLayer::ccTouchMoved(cocos2d::CCTouch *touch,
                                   cocos2d::CCEvent *event) {
...
bool ButtonEditLayer::init() {
    ...
    btn = CCMenuItemExt::createSpriteExtra(
        spr, [this](CCMenuItemSpriteExtra *sender) {
            ButtonEditLayer::onClose(sender);
        });
    btn->setPosition({-40, -45});
    menu->addChild(btn);
    ...
    // menu_selector(ButtonEditLayer::onRestore));
    btn = CCMenuItemExt::createSpriteExtra(
        spr, [this](CCMenuItemSpriteExtra *sender) {
            ...
            g.mod->setSavedValue("button_speedhack_pos_x",
                                 winSize.width - 62.f);
            g.mod->setSavedValue("button_speedhack_pos_y",
                                 winSize.height - 38.f);
            g.mod->setSavedValue("button_speedhack_scale", 1.f);
            g.mod->setSavedValue("button_speedhack_opacity", 1.f);
...
void ButtonEditLayer::addSliders() {
    // Opacity Slider
    opacitySlider = SliderNode::create([this](SliderNode *sender, float value) {
        spriteButtons[currentSelected]->setOpacity(
    ...
    opacitySlider->setScale(0.5f);        // Shrink the whole node
    opacitySlider->setPosition({5, -10}); // Relative to menu center
    opacitySlider->setValue(opacities[indexToID[currentSelected]]);
    menu->addChild(opacitySlider);

    // Scale Slider
    scaleSlider = SliderNode::create([this](SliderNode *sender, float value) {
    ...
    scaleSlider->setPosition({5, 13}); // Relative to menu center
    scaleSlider->setValue(scales[indexToID[currentSelected]]);
    menu->addChild(scaleSlider);
}

// void ButtonEditLayer::addSliders() {
//     opacitySlider = SliderNode::create(
//         [this](SliderNode* sender, float value) {
//             spriteButtons[currentSelected]->setOpacity(static_cast<int>(value
//             * 255));
...
//     opacitySlider->getThumb()->setScale(0.8f);
//     opacitySlider->setScale(0.5f);
//     opacitySlider->setValue(opacities[indexToID[currentSelected]]);
//     menu->addChild(opacitySlider);

//     scaleSlider = SliderNode::create(
//         [this](SliderNode* sender, float value) {
//             spriteButtons[currentSelected]->setScale(value);
...
//     scaleSlider->getThumb()->setScale(0.8f);
//     scaleSlider->setScale(0.5f);
//     scaleSlider->setValue(scales[indexToID[currentSelected]]);
//     menu->addChild(scaleSlider);
// }
...
void ButtonEditLayer::addSprites() {
    ...
    spr->setOpacity(static_cast<int>(
        mod->getSavedValue<float>("button_off_opacity") * 255));
