...
void LoadMacroLayer::onImportMacro(CCObject*) {
    ...
    m_importTask.spawn(file::pick(file::PickMode::OpenFile, fileOptions), [weakThis](auto res) {
        ...
        std::string name =
            geode::utils::string::pathToString(isGdrJson ? path.stem().stem() : path.stem());
        ...
        if (!needsConversion) {
            ...
            std::filesystem::copy_file(
                path, finalPath, std::filesystem::copy_options::overwrite_existing, ec);
            if (ec) {
...
bool LoadMacroLayer::init(geode::Popup* layer, geode::Popup* layer2, bool autosaves) {
    ...
    if (!isMerge) {
        ...
        CCMenuItemSpriteExtra* btn =
            CCMenuItemSpriteExtra::create(icon, this, menu_selector(LoadMacroLayer::onImportMacro));
    ...
    sortToggle =
        CCMenuItemToggler::create(spr1, spr3, this, menu_selector(LoadMacroLayer::updateSort));
    ...
    spr->setScale(0.685f);
    searchOff =
        CCMenuItemSpriteExtra::create(spr, this, menu_selector(LoadMacroLayer::clearSearch));
...
bool MacroCell::init(std::filesystem::path path,
    ...
    CCMenuItemSpriteExtra* btn =
        CCMenuItemSpriteExtra::create(spr, this, menu_selector(MacroCell::onLoad));
    ...
    btn = CCMenuItemSpriteExtra::create(spr2, this, menu_selector(MacroCell::onDelete));
    ...
    toggler =
        CCMenuItemToggler::create(spriteOff, spriteOn, this, menu_selector(MacroCell::onSelect));
...
void MacroCell::handleLoad() {
    ...
    if (!newLayer)
        newLayer = g.layer != nullptr ? static_cast<RecordLayer*>(g.layer) : nullptr;
    if (newLayer)
        newLayer->updateTPS();
