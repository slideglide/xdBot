...
class $modify(GJBaseGameLayer) {
    ...
    void processCommands(float dt, bool isHalfTick, bool isLastTick) {
        ...
        for (int i = 0; i < 2; i++) { // check 2 player mode
            bool isPlayer1 = (i == 0);
            if ((!g.autoclickerP1 && isPlayer1) || (!g.autoclickerP2 && !isPlayer1))
                continue;
            ...
            if (f->framesHolding[i] < holdFor[i]) {
                if (f->framesHolding[i] == 0) {
                    ...
                    GJBaseGameLayer::handleButton(
                        true, 1, Macro::flipControls() ? !isPlayer1 : isPlayer1);
                    f->autoclick[i] = false;
                }
                ...
                f->autoclick[i] = true;
                GJBaseGameLayer::handleButton(
                    false, 1, Macro::flipControls() ? !isPlayer1 : isPlayer1);
                f->autoclick[i] = false;
    ...
    void handleButton(bool hold, int button, bool player1) {
        ...
        if (f->autoclick[i] || (realPlayer1 && !g.autoclickerP1) ||
            (!realPlayer1 && !g.autoclickerP2))
            return GJBaseGameLayer::handleButton(hold, button, player1);
