...
const std::vector<IncompatibleMod> incompatibleMods{
    ...
    {"thesillydoggo.qolmod",
     true,
     {{"tps-bypass_enabled", true, false, true}}}};
...
bool Global::hasIncompatibleMods() {
    ...
    for (IncompatibleMod incompatMod : incompatibleMods) {
        ...
        for (IncompatibleSetting sett : incompatMod.incompatSettings) {
            bool value = sett.isSavedValue
                             ? mod->getSavedValue<bool>(sett.ID)
            ...
            if (sett.isModToggle)
                ...
                settingsToDisable.push_back(
                    fmt::format("{} ({})", settName, modName));
            }
        }
    }
    ...
    if (!modsToDisable.empty()) {
        ...
        for (const std::string name : modsToDisable)
            incompatString.append("<cr>{}</c>{}", name,
...
bool Global::enabledIncompatibleGDSettings() {
    ...
    if (cbsOverridden) {
        settingsToDisable.push_back(
            "Click Between Steps (Level Settings Override)");
    }
#endif
...
void Global::updateSeed(bool isRestart) {
    ...
    if (g.seedEnabled) {
        ...
        if (!pl->m_player1->m_isDead) {
            g.gen.seed(seed + static_cast<uint64_t>(
    ...
    if (!g.frameStepper) {
        ...
        if (PlayLayer::get() && g.frameStepperMusicTime != 0) {
            FMODAudioEngine::sharedEngine()->setMusicTimeMS(
                g.frameStepperMusicTime, true, 0);
...
$execute {
    ...
    if (!g.mod->setSavedValue("defaults_set_14", true)) {
        g.mod->setSavedValue("render_fade_in_video",
                             geode::utils::numToString(2));
        g.mod->setSavedValue("render_fade_out_video",
                             geode::utils::numToString(2));
    }
    ...
    if (!g.mod->setSavedValue("defaults_set_10", true)) {
        ...
        g.mod->setSavedValue("autosave_interval",
                             geode::utils::numToString(10));
        ...
        g.mod->setSavedValue("trajectory_length",
                             geode::utils::numToString(240));
    }
    ...
    if (!g.mod->setSavedValue("defaults_set3", true)) {
        ...
        g.mod->setSavedValue("render_seconds_after",
                             geode::utils::numToString(2));
    ...
    if (Loader::get()->getLoadedMod("eclipse.eclipse-menu")) {
        geode::queueInMainThread([&g] {
            eclipse::config::setInternal("global.tpsbypass.toggle",
                                         g.tpsEnabled);
    ...
    g.macro.gameVersion = static_cast<int>(
